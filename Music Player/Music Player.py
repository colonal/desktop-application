...
from StyleMusicPlayer import style
import sqlite3
...
def  Convert(src, Id):
    global musicList, _historyCount, _Convert, _historyTime
    dst = src.split(".")[0] + '.mp3'
...
class Player(QMainWindow):
    ...
    def style(self):
        ...
        self.volumeSlider.setStyleSheet(style.Slider())
    ...
    def AddSound(self):
        file = "Sound Files (*.mp3 *.ogg *.wav *.mp4 *.wmv *.aac *.wma) ;;All Files (*)"
    ...
    def  Folder(self):
        ...
        for I in name :
            ...
            if x == "mp3" or x == "wav" or x == "mp4" or x == "wmv" or x == "aac" or x == "wma" or x == "ogg":
                fileName = os.path.basename(I)
                self.playList.addItem(fileName)
    ...
    def playSounds(self):
        global songLength, count, ind, nameplay, _historyCount,sliderChangeStop,_historyTime
            ...
            if  "Unknown WAVE data format" in str(Error) or "ModPlug_Load failed" in str(Error):
                ...
                if os.path.exists(dst):
                    ...
                    if mss == QMessageBox.Yes:
                        src = str(musicList[index])
                        file = ["mp3","ogg" ,"wav" ,"mp4" ,"wmv" ,"aac" ,"wma"]
    ...
    def runConvert(self, Dst,  Id ):
        global _Convert
        while True:
            if _Convert == True:
                _Convert = False
                break
        self.playList.setCurrentRow(Id)
    ...
    def run_convert(self, src, index):
        x = threading.Thread(target=Convert, args=(src, index))
        x.start()
        x.join()
        
        dst = src.split(".")[0] + '.mp3'
        return self.runConvert(dst, index)


    def run_CuntVolume(self):
        try:
            threading.Thread(target=self.CuntVolume).start() 
        except:
            pass
    ...
    def About(self):
        ...
        help_message.setStyleSheet(style.QMessgesBox())
    ...
    def Githop(self):
        ...
        Mess.setStyleSheet(style.QMessgesBox())
    ...
    def dropEvent(self, event):
        if event.mimeData().hasImage:
            ...
            if "." in file_path.split("\\")[-1]: 
                ...
                if i == "mp3" or i == "wav" or i == "mp4" or i == "wmv" or i == "aac" or i == "wma" or i == "ogg":
